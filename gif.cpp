// gif.cpp
// Implementation of GIF encoder functions
// Public domain by Charlie Tangora
...
void GifSwapPixels(uint8_t* image, int pixA, int pixB)
{
    for(int i=0;i<4;i++)
    {
        uint8_t tmp = image[pixA*4+i];
        image[pixA*4+i] = image[pixB*4+i];
        image[pixB*4+i] = tmp;
    }
}
...
int GifPartition(uint8_t* image, const int left, const int right, const int elt, int pivotValue)
{
    ...
    for(int ii=left; ii<right; ++ii)
    {
        ...
        if(arrayVal < pivotValue || (arrayVal == pivotValue && split))
        {
...
// Additional functions like GifMakePalette, GifDitherImage, GifWriteLzwImage, GifBegin, GifWriteFrame, GifEnd
// would follow exactly the same translation pattern.
